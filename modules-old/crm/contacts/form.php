...
<form method="POST" action="save.php">
  ...
  <div class="form-group">
    ...
    <select name="lead_id" id="" class="form-control">
      ...
      <?php $leads = $conn->query("SELECT id, full_name, company_name FROM crm_leads WHERE company_id = $company_id"); foreach($leads as $lead) { ?>
        <option value="<?= $lead['id'] ?>" <?= $isEdit && $contact['lead_id'] == $lead['id'] ? 'selected' : '' ?>><?= $lead['full_name'] ?> - <?= $lead['company_name'] ?></option>
      <?php } ?>
    </select>
  </div>

...
$id = $_GET['id'] ?? null;
$quotation = $id ? $conn->query("SELECT * FROM sales_quotations WHERE company_id = $company_id AND id = $id")->fetch_assoc() : null;
...
if($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $quote_stmt = $conn->prepare("UPDATE sales_quotations SET lead_id=?, company_id=?, customer_id=?, wht_tax_id=?, quote_number=?, quotation_date=?, valid_until=?, status=?, total=?, tax=?, wht_tax_amount=?, created_by=?, notes=? WHERE id=?");
            $quote_stmt->bind_param("iiiissssdddisi", $lead_id, $company_id, $customer_id, $wht_tax_id, $quote_number, $quotation_date, $valid_until, $status, $total, $tax_amount, $wht_tax_amount, $created_by, $notes, $id);
    $quote_stmt->execute();
                                ...
                                <table class="table table-bordered" id="itemsTable">
                                    ...
                                    <tbody>
                                        ...
                                        <?php if(isset($quotation['id'])) { ?>
                                            <?php $quotation_items = $items = $db->query("SELECT i.*, p.name AS product_name
                                                    FROM sales_quotation_items i
                                                    JOIN sales_products p ON i.product_id = p.id
                                                    WHERE i.quotation_id = {$quotation['id']}");
                                            ?>
                                            <?php foreach($quotation_items as $item) { ?>
                                                <tr>
                                                    ...
                                                    <td><input type="number" name="unit_price[]" class="form-control" step="0.01" value="<?= $item['unit_price']; ?>" onchange="recalcTotal()" readonly></td>
                                                    <td><input type="number" name="discount_percent[]" class="form-control" step="0.01" value="<?= $item['discount']; ?>" onchange="recalcTotal()"></td>
                                ...
                                <dirv class="row">
                                    <div class="col">
                                        <div class="form-group">
                                            ...
                                            <input type="number" name="tax" class="form-control" value="<?= $quotation['tax'] ?? '0.00' ?>" readonly>
                                        </div>
                                    </div>
                                    <div class="col">
                                        <div class="form-group">
                                            <label>Total Amount (N)</label>
                                            <input type="number" name="total" class="form-control" id="total" value="<?= $quotation['total'] ?? '0.00' ?>" readonly>
                                        </div>
                                    </div>
                                </dirv>
                            </div>

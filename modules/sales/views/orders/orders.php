...
function handle_order_form_submission() {
    ...
    $vat_tax_amount = $_POST['vat_tax_amount'] ?? 0;
    $wht_tax_amount = $_POST['wht_tax_amount'] ?? 0;
    $total_amount = $_POST['total_amount'] ?? 0;

    if ($id) {
        $update_order_stmt = $db->prepare("UPDATE sales_orders SET company_id=?, user_id=?, employee_id=?, customer_id=?, wht_tax_id=?, order_date=?, delivery_date=?, status=?, notes=?, vat_tax_amount=?, wht_tax_amount=?, total_amount=? WHERE id=?");
        $update_order = $update_order_stmt->execute([$company_id, $user_id, $employee_id, $customer_id, $wht_tax_id, $order_date, $delivery_date, $status, $notes, $vat_tax_amount, $wht_tax_amount, $total_amount, $id]);
        if($update_order) {
        ...
        $add_order = $db->query("INSERT INTO sales_orders (company_id, user_id, employee_id, order_number, customer_id, wht_tax_id, order_date, delivery_date, status, notes, vat_tax_amount, wht_tax_amount, total_amount) 
                    VALUES ($company_id, $user_id, $employee_id, '$order_number', '$customer_id', $wht_tax_id, '$order_date', '$delivery_date', '$status', '$notes', '$vat_tax_amount', '$wht_tax_amount', '$total_amount')");

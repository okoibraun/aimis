...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['new_version']) && $_FILES['new_version']['error'] === 0) {
        ...
        if (move_uploaded_file($_FILES['new_version']['tmp_name'], $destination)) {
            ...
            $conn->prepare("INSERT INTO document_versions (company_id, document_id, version_number, file_path, file_type, created_by)
                           VALUES (?, ?, ?, ?, ?, ?)")
                ->execute([
                    $doc['company_id'], $doc['id'], $doc['version'], $doc['file_path'], $doc['file_type'], $_SESSION['user_id']
                ]);

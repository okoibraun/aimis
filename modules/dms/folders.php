...
$super_role = super_roles();
// Check if user has permission to manage folders
if(!in_array($_SESSION['user_role'], $super_role) && !in_array($page, $user_permissions)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add') {
    ...
    $stmt = $conn->prepare("INSERT INTO document_folders (company_id, name, parent_id, created_by) VALUES (?, ?, ?, ?)");
    $stmt->execute([$company_id, $name, $parent_id, $user_id]);
    header("Location: folders.php?added=1");
    exit;
}

// Handle delete
if(in_array($_SESSION['user_role'], $super_role)) {
    if (isset($_GET['delete'])) {
        // $delete_id = (int)$_GET['delete'];
        $delete_id = intval($_GET['delete']);
        $conn->prepare("DELETE FROM document_folders WHERE id = ? AND company_id = ?")->execute([$delete_id, $company_id]);
    ...
    if (isset($_GET['delete'])) {
        //$delete_id = (int)$_GET['delete'];
        $delete_id = intval($_GET['delete']);
        $conn->prepare("DELETE FROM document_folders WHERE id = ? AND company_id = ? AND created_by = ?")->execute([$delete_id, $company_id, $user_id]);
        header("Location: folders.php?deleted=1");
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'rename') {
    ...
    $conn->prepare("UPDATE document_folders SET name = ? WHERE id = ? AND company_id = ? AND created_by = ?")->execute([$new_name, $folder_id, $company_id, $user_id]);
    header("Location: folders.php?renamed=1");
    exit;
}

// Fetch folders
$folders = $conn->query("SELECT * FROM document_folders WHERE company_id = $company_id AND created_by = $user_id ORDER BY parent_id ASC, name ASC");
$folder_tree = [];

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($sql) {
        ...
        if ($run) {
            ...
            // Log query
            $stmt = $conn->prepare("INSERT INTO ai_nl_queries (user_id, query_text, response_text, executed_sql) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("isss", $user_id, $user_query, $response_text, $sql);
            $stmt->execute();
        } else {
            $error = "Error in SQL execution: " . mysqli_error($conn);
        }
    } else {
            ...
            <div class="content-wrapper">
                ...
                <section class="content">
                    ...
                    <div class="alert alert-success">
                        <strong>AI Response:</strong> <?= htmlspecialchars($response_text) ?>
                    </div>
                    <?php endif; ?>

                    <?php if (!empty($result_data)): ?>
                    <div class="card card-primary">
                        <div class="card-header"><h3 class="card-title">Query Result</h3></div>

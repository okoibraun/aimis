...
$rule = $conn->query("SELECT * FROM intl_tax_rules WHERE id = $id AND company_id = $company_id")->fetch_Assoc();
?>
<!doctype html>
<html lang="en">
  <!--begin::Head-->
            ...
            <div class="content-wrapper">
              ...
              <section class="content mt-3">
                <form action="save.php" method="POST" class="card">
                  <input type="hidden" name="id" id="rule_id" value="<?= $rule['id'] ?>">
                  ...
                  <div class="card-body">
                    <div class="form-group">
                      <label>Country</label>
                      <input type="text" name="country" id="rule_country" class="form-control" value="<?= $rule['country'] ?>" required>
                    </div>
                    <div class="form-group">
                      <label>Template Name</label>
                      <input type="text" name="template_name" id="rule_template" class="form-control" value="<?= $rule['template_name'] ?>" required>
                    </div>
                    <div class="form-group">
                      <label>BEPS Compliant?</label>
                      <select name="beps_compliant" id="rule_beps" class="form-control">
                        <option value="1" <?= $rule['beps_compliant'] == 1 ? 'selected' : '' ?>>Yes</option>
                        <option value="0" <?= $rule['beps_compliant'] == 0 ? 'selected' : '' ?>>No</option>
                      </select>
                    </div>
                    <div class="form-group">
                      <label>Status</label>

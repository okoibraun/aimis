...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  $stmt = $conn->prepare("INSERT INTO accounts (company_id, account_code, account_name, account_type, parent_account_id) VALUES (?, ?, ?, ?, ?)");
  $stmt->bind_param("isssi", $company_id, $code, $name, $type, $parent);
  $stmt->execute();
  header("Location: ./");
  exit();
}
                    ...
                    <section class="content col-lg-6">
                      <div class="card">
                        <div class="card-header">
                          <div class="card-title">New Account Details</div>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="form-group">
                              ...
                              <select name="parent_account_id" class="form-control">
                                ...
                                while ($r = mysqli_fetch_assoc($res)) {
                                  echo "<option value='{$r['id']}'>{$r['account_name']}</option>";
                                }
                                ?>
                              </select>
                            </div>
